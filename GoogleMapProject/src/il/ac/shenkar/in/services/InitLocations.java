...
public class InitLocations extends AsyncTask<Context, Integer, CampusInLocations>
{
	...
	try
	{
	    ...
	    // URL url = new
	    // URL("	http://globalwork.co.il/uploadFiles/example");
	    URL url = new URL("https://studentportal-jscapps.rhcloud.com/CampusIn/LocationConfig.xml");
	...
	{
	    // if the file does not exists meaning the DB is empty and we need
	    // to init it
	    ...
	    {
		Log.e("CampusIn", "error uccered while parsing the downloaded xml: " + e.getMessage());
	    }
	}
	else
	{
	    /**
	     * else mean there is a file existing we need to distinguish between
	     * two cases: 1 the file is the same file -> in this case do nothing
		...
		// DB is updated change the file name from downloaded to
		// existing
		outputFile.renameTo(existingXML);
